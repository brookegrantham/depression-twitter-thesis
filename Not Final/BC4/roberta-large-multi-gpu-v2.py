...
from transformers import RobertaForSequenceClassification, RobertaTokenizer, RobertaConfig, AutoTokenizer, AutoModelForSequenceClassification
...
roberta_model = AutoModelForSequenceClassification.from_pretrained(PRETRAINED_MODEL_NAME, num_labels=3)
roberta_tok = AutoTokenizer.from_pretrained(PRETRAINED_MODEL_NAME)
...
def train_model(model, data_loader, loss_fn, optimizer, scheduler, n_examples):
    ...
    for step,d in enumerate(data_loader):
        ...
        loss = loss_fn(outputs, targets.to('cuda:0'))

        correct_predictions += torch.sum(preds == targets.to('cuda:0'))
